...
class User(UserMixin):
    def __init__(self, user_id, login, role_id, first_name, last_name):
        super().__init__()
        self.id=user_id
        self.login=login
...
def load_user(user_id):
    ...
    cursor.close()
    if db_user:
            return User(user_id=db_user.id, login=db_user.login, role_id=db_user.role_id, first_name = db_user.first_name, last_name=db_user.last_name)
    return None   
...
def login():
    if request.method=='POST':
        ...
        if login and password:
            ...
            cursor.close()
            if db_user:
                    user= User(user_id=db_user.id, login=db_user.login, role_id=db_user.role_id, first_name = db_user.first_name, last_name=db_user.last_name)
